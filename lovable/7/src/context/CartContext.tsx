...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const getKey = (id: string, size?: string) => `${id}-${size || "default"}`;
  ...
  const removeItem = useCallback((productId: string, size?: string) => {
    setItems((prev) =>
      prev.filter((i) => getKey(i.product.id, i.size) !== getKey(productId, size))
    );
  }, []);
  ...
  const updateQuantity = useCallback(
    (productId: string, quantity: number, size?: string) => {
      ...
      setItems((prev) =>
        prev.map((i) =>
          getKey(i.product.id, i.size) === getKey(productId, size)
            ? { ...i, quantity }
            : i
        )
      );
